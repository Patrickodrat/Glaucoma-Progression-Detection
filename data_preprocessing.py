...
AA_WEIGHTS_PATH = "aa_weights.npy"

if os.path.exists(AA_WEIGHTS_PATH):
    import numpy as np
    archetype_weights = np.load(AA_WEIGHTS_PATH)       # shape (N_samples, K)
    assert archetype_weights.shape[0] == len(datalist), \
        f"AA weights N={archetype_weights.shape[0]} does not match datalist N={len(datalist)}"
    print("Loaded archetype_weights with shape:", archetype_weights.shape)
else:
    archetype_weights = None
    print("WARNING: aa_weights.npy not found. Run compute_aa_weights.py first.")
