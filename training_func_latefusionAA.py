...
from Model import GPTS_LateFusionAA
...
os.makedirs("./saved_models_GPTS_LateFusionAA", exist_ok=True)
...
for train_idx_rel, val_idx_rel in skf.split(
    ...
    model = GPTS_LateFusionAA(num_archetypes=NUM_ARCHETYPES).to(device)
    ...
    if best_state_dict is not None:
        torch.save(best_state_dict,
                   f'./saved_models_GPTS_LateFusionAA/best_model_fold_{fold_idx}.pth')
...
torch.save(best_state_for_test, "./saved_models_GPTS_LateFusionAA/best_model_for_test.pth")
print(f"\nBest fold for test is fold {best_fold_index+1} "
      f"with val_acc={fold_best_val_accs[best_fold_index]:.4f}")
print("Saved as saved_models_GPTS_LateFusionAA/best_model_for_test.pth")
